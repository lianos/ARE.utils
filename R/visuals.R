...
my.image.matrix <- function(m, ...) {
  ...
  nr <- nrow(m)
  nc <- ncol(m)
  image.default(1:nc, 1:nr, t(m)[, nr:1], ...)
}
...
plot.densities <- function(..., along=2, density.params=list(), 
  ...
  #                :   - a list of densities
  #                :   - a list of numbers
  #                :   - a Nx2 matrix of data, where each row/column is a separate
  #                :     set of observations. Specify which dimension to use by 
  #                :     `along`
  ...
  #                : Note that passing nothing here will simply use the default args
  ...
  if (is.list(data[[1]]) && all(sapply(data[[1]], is, 'density'))) {
    data <- data[[1]]
  } else {
  ...
  stopifnot(all(sapply(data, is, 'density')))
  
  if (is.null(plot.params$col) || length(plot.params$col) < length(data)) {
    plot.params$col <- rainbow(length(data))
  }
  ...
  if (is.character(legend) && length(legend) == length(data)) {
    legend('topright', legend=legend, text.col=plot.params$col)
  }
