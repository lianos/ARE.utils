...
heatmap.22 <- function(x,
                       ...
                       denscol=tracecol,
                       symkey = min(x, na.rm=TRUE) < 0 || symbreaks,
                       densadj = 0.25,
                       ...
                       ## steve
                       trim4key=FALSE, # logical or numeric
                       ...
                       )
{
  ...
  if (is.logical(labRow) && !labRow) {
    ## STEVE : Added labRow=FALSE to not draw label on axis at all
    labRow <- FALSE
  ...
  if (is.logical(labCol) && !labCol) {
    ## STEVE : Added labCol=FALSE to not draw label on axis at all
    labCol <- FALSE
  ...
  if(length(breaks)==1)
    {
      if(!symbreaks)
        ...
        {
          ########################################## STEVE
          if (is.logical(trim4key) && trim4key) {
            .x.tmp <- trim.data(x)
          } else if (is.numeric(trim4key)) {
            .x.tmp <- trim.data(x, trim4key)
          }
          if (exists('.x.tmp')) {
            cat("Truncated", sum(!.x.tmp %~% x), "values for visual\n")
            x <- .x.tmp
            rm(.x.tmp)
          }
          ################################################
